 ...
 */
PostgresJSONQueryStrategy.prototype.composeSingle = function(serializedCriteria, previousOutput) {
    if (!previousOutput) { 
        previousOutput = {
            lastPosition: 0,
    ...
    if (serializedCriteria.content) {
        for (var i=0; i<serializedCriteria.content.length; i++) {
            ...
            if (element.pivotDataType) {
                ...
                previousOutput.commonTableExpressions.push(previousOutput.statement); 
                console.log(previousOutput);
                /*
                previousOutput.commonTableExpressions.push(subquery.statement);

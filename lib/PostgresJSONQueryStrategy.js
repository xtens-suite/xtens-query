...
function PostgresJSONQueryStrategy() {
    ...
    this.composeLoopStatement = function(element, lastParameterPosition, loopCount) {
        ...
        for (var i=0; i<element.content.length; i++) {
            ...
            subquery = "SELECT "+subqueryInsertion+"row_number() OVER () AS rid, list_"+i+".value::text AS attribute_"+i+"_value";
            subquery += ", list_"+i+".unit::text AS attribute_"+i+"_unit FROM ";
            ...
            constraints.push("attribute_" + i + "_value::" + field.fieldType.toLowerCase() + " " + field.comparator + " " + valueParam);
            if (field.fieldUnit) {
                unitParam = '$'+(++lastParameterPosition);
                constraints.push("attribute_" + i + "_unit LIKE " + unitParam);
            }
            notFirst = true;
        }    
        commonTableExpr += ")";
    ...
    if (serializedCriteria.content) {
        ...
        for (var i=0; i<serializedCriteria.content.length; i++) {
            ...
            if (element.loopName) {
                ...
                if (i < serializedCriteria.content.length-1) {
                    query += " AND ";
                }
            }
        }

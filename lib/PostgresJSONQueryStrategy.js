...
function PostgresJSONQueryStrategy() {
    ...
    this.composeSpecializedSubjectQuery = function(subjectCriteria, lastParameterPosition) {
        if (!lastParameterPosition) {
            lastParameterPosition = 0;
        }
        var query = {}, clauses = [];
        var subjProperties = ["code", "sex"];
        subjProperties.forEach(function(property) {
            if (subjectCriteria[property]) {
                var comparator = allowedComparators.indexOf(subjectCriteria[property+"Comparator"]) >= 0 ? subjectCriteria[property+"Comparator"] : '=';
                clauses.push(property.toUnderscore() + " " + comparator + " $" + (++lastParameterPosition));
    ...
    this.composeSpecializedPersonalDetailsQuery = function(pdCriteria, previousOutput) {
        if (!previousOutput) {
            ...
            var pdProperties = ["surname", "givenName", "birthDate"];
            ...
            /*
               if (pdCriteria.surname) {
               comparator = allowedComparators.indexOf(pdCriteria.surnameComparator) >= 0 ? pdCriteria.surnameComparator : 'LIKE';
               whereClauses.push("surname " + comparator + " $" + (++previousOutput.lastPosition));
               previousOutput.parameters.push(pdCriteria.surname);
               }
               if (pdCriteria.givenName) {
               comparator = allowedComparators.indexOf(pdCriteria.givenNameComparator) >= 0 ? pdCriteria.givenNameComparator : 'LIKE';
               whereClauses.push("given_name " + comparator + " $" + (++previousOutput.lastPosition));
               previousOutput.parameters.push(pdCriteria.givenName);
               }
               if (pdCriteria.birthDate) {
               comparator = allowedComparators.indexOf(pdCriteria.birthDateComparator) >= 0 ? pdCriteria.birthDateComparator : '=';
               whereClauses.push("birth_date " + comparator + " $" + (++previousOutput.lastPosition));
               previousOutput.parameters.push(pdCriteria.birthDate);
               } */

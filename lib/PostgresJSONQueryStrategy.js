...
var allowedComparators = ['=', '<', '>', '<=', '>=', '<>', 'IN', 'NOT IN', 'LIKE', 'NOT LIKE'];
var specializedProperties = {};
specializedProperties[DataTypeClasses.SUBJECT] = ["code", "sex"];
specializedProperties[DataTypeClasses.SAMPLE] = ["biobankCode"];
// var subjProperties = ["code", "sex"]; //list of specialized properties in the SUBJECT table
...
function PostgresJSONQueryStrategy() {
    ...
    this.composeSpecializedQuery = function(criteria, previousOutput, tablePrefix) {
        /*
        var res;
           switch(criteria.specializedQuery) {
           case DataTypeClasses.SUBJECT:
           res = this.composeSpecializedSubjectQuery(criteria, previousOutput.lastPosition, tablePrefix);
           subjProperties.forEach(function(property) {
           previousOutput.parameters.push(criteria[property]);
           });
           res.parameters = previousOutput.parameters;
           return res;
           case DataTypeClasses.SAMPLE:
           res = this.composeSpecializedSampleQuery(criteria, previousOutput.lastPosition, tablePrefix);
           default:
           return null;
           }
           
        res = this.composeSpecializedSubjectQuery(criteria, previousOutput.lastPosition, tablePrefix, criteria.specializedQuery);
        */
        ...
        specializedProperties[dataTypeClass].forEach(function(property) {
            if (criteria[property]) {
                if (_.isArray(criteria[property])) { // if it is a list of options (like in sex)
                    ...
                    var values = [];
                    for (var i=0; i<criteria[property].length; i++) {
        ...
        /*
        specializedProperties[criteria.specializedQuery].forEach(function(property) {
            previousOutput.parameters.push(criteria[property]);
        }); */
    ...
    /*
    this.composeSpecializedSubjectQuery = function(subjectCriteria, lastParameterPosition, tablePrefix, dataTypeClass) {
        if (!lastParameterPosition) {
            lastParameterPosition = 0;
        }
        tablePrefix = tablePrefix || ''; //'d.';
        var query = {}, clauses = [], comparator;
        specializedProperties[dataTypeClass].forEach(function(property) {
            if (subjectCriteria[property]) {
                if (_.isArray(subjectCriteria[property])) { // if it is a list of options (like in sex)
                    comparator = allowedComparators.indexOf(subjectCriteria[property+"Comparator"]) >= 0 ? subjectCriteria[property+"Comparator"] : 'IN';
                    var values = [];
                    for (var i=0; i<subjectCriteria[property].length; i++) {
                        values.push('$'+(++lastParameterPosition));
                    }
                    clauses.push(tablePrefix + property.toUnderscore() + " " + comparator + " (" + values.join() + ")");
                }
                else {
                    comparator = allowedComparators.indexOf(subjectCriteria[property+"Comparator"]) >= 0 ? subjectCriteria[property+"Comparator"] : '=';
                    clauses.push(tablePrefix + property.toUnderscore() + " " + comparator + " $" + (++lastParameterPosition));
                }
                //TODO How to handle properly paramaters? 
    ...
    this.composeSpecializedPersonalDetailsQuery = function(pdCriteria, previousOutput) {
        if (!previousOutput) {
            previousOutput = {
                lastPosition: 0,
                cteCount: 0,
            ...
            var query = { alias: 'pd'};
            query.select = "SELECT * FROM personal_details";
            query.where = "";
            var whereClauses = []; // comparator;

            pdProperties.forEach(function(property) {
                if (pdCriteria[property]) {
                    var comparator = allowedComparators.indexOf(pdCriteria[property+"Comparator"]) >= 0 ? pdCriteria[property+"Comparator"] : '=';
    ...
    this.composeCommonTableExpression = function(query, ctes, parentAlias, parentTable) {
        if (!ctes) {
            ...
            ctes.push({
                ...
                joinClause: "INNER JOIN " + query.alias + " ON " + query.alias + ".id = " + parentAlias + ".personal_info"
            });
            return ctes;
        }
        else {
...
PostgresJSONQueryStrategy.prototype.composeSingle = function(serializedCriteria, previousOutput, query) { // should I pass the parent params??
    ...
    if (serializedCriteria.content) {
        for (var i=0; i<serializedCriteria.content.length; i++) {
            var res, op, element = serializedCriteria.content[i];
            if (element.pivotDataType) {
                ...
                op = this.getSubqueryRow(element, previousOutput.lastPosition, tablePrefix);
                if (!op) {
                    continue;
                }
                fieldQueries.push(op.subquery);
    ...
    if (fieldQueries.length) {
        ...
        query.where += " AND (" + _.compact(fieldQueries).map(function(row) {return "(" + row + ")"; }).join(" " + junction + " ") + ")";
    } 
